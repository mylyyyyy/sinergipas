...
<head>
    ...
    <style>
        :root {
            ...
            --font-display: "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", ui-sans-serif, system-ui, sans-serif;
            --font-ui: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif;
            --font-data: Roboto, "Segoe UI", "Helvetica Neue", Arial, "Noto Sans", ui-sans-serif, system-ui, sans-serif;
            --font-caption: "Segoe UI", Roboto, Arial, sans-serif;
        }
        body { 
            font-family: var(--font-body);
            background:
                radial-gradient(circle at top right, rgba(59, 130, 246, 0.12), transparent 28%),
                radial-gradient(circle at bottom left, rgba(234, 179, 8, 0.12), transparent 28%),
                #F1F5F9;
        }
...
<body class="antialiased min-h-screen flex items-center justify-center p-6 overflow-hidden">
    <!-- Abstract Shapes -->
    <div class="absolute top-[-10%] -right-[10%] w-96 h-96 bg-[#EAB308] opacity-10 rounded-full blur-3xl animate-float"></div>
    <div class="absolute bottom-[-10%] -left-[10%] w-80 h-80 bg-[#3B82F6] opacity-10 rounded-full blur-3xl animate-float" style="animation-delay: 2s"></div>
    ...
    <div class="w-full max-w-[480px] relative z-10">
        ...
        <div class="text-center mb-12">
            ...
            <h1 class="text-3xl font-extrabold text-[#0F172A] tracking-tight">SINERGI PAS</h1>
        ...
        <div class="glass-card p-12 rounded-[48px] shadow-2xl shadow-gray-200/50">
            <form action="{{ route('login.post') }}" method="POST" class="space-y-8">
                ...
                <div class="space-y-3">
                    <label class="text-xs font-extrabold text-[#0F172A] uppercase tracking-[0.2em] ml-1">Email Kantor</label>
                    <input type="email" name="email" required 
                        class="w-full px-6 py-4 rounded-[20px] border border-[#EFEFEF] bg-white/50 text-[#0F172A] focus:ring-4 focus:ring-red-500/10 focus:border-[#EAB308] outline-none transition-all placeholder:text-[#ABABAB] font-medium"
                        placeholder="username@pas.id">
                </div>

                <div class="space-y-3">
                    <div class="flex justify-between items-center ml-1">
                        <label class="text-xs font-extrabold text-[#0F172A] uppercase tracking-[0.2em]">Password</label>
                        <a href="{{ route('password.request') }}" class="text-xs font-bold text-[#EAB308] hover:text-[#CA8A04] transition-colors">Lupa Password?</a>
                    </div>
                    <div class="relative group">
                        <input type="password" name="password" id="password" required 
                            class="w-full px-6 py-4 rounded-[20px] border border-[#EFEFEF] bg-white/50 text-[#0F172A] focus:ring-4 focus:ring-red-500/10 focus:border-[#EAB308] outline-none transition-all placeholder:text-[#ABABAB] font-medium pr-14"
                            placeholder="••••••••••••">
                        <button type="button" onclick="togglePassword()" class="absolute right-5 top-1/2 -translate-y-1/2 text-[#ABABAB] hover:text-[#EAB308] transition-colors focus:outline-none">
                ...
                <button type="submit" 
                    class="w-full bg-[#EAB308] text-white py-5 rounded-[24px] font-extrabold text-lg hover:bg-[#CA8A04] transition-all shadow-xl shadow-red-200 active:scale-[0.98] flex items-center justify-center gap-3">
                    Masuk ke Sistem
