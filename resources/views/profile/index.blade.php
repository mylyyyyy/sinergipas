...
<div class="max-w-5xl mx-auto">
    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-[56px] border border-[#EFEFEF] shadow-xl shadow-gray-100/50 overflow-hidden mb-10 transition-all duration-500 hover:shadow-2xl">
            <!-- Decorative Header -->
            <div class="h-48 bg-gradient-to-r from-[#0F172A] to-[#EAB308] relative">
            ...
            <div class="px-12 pb-12">
                <div class="relative -mt-20 mb-10 flex flex-col md:flex-row items-end gap-8">
                    ...
                    <div class="relative group">
                        <div class="w-40 h-40 rounded-[40px] border-[6px] border-white bg-[#F1F5F9] overflow-hidden shadow-2xl flex items-center justify-center text-[#8A8A8A]">
                        ...
                        <label for="photoInput" class="absolute -bottom-2 -right-2 bg-[#EAB308] p-3 rounded-2xl shadow-xl cursor-pointer hover:bg-[#CA8A04] transition-all hover:scale-110 active:scale-95 border-4 border-white">
                    ...
                    <div class="flex-1 pb-2">
                        <h2 class="text-3xl font-black text-[#0F172A] tracking-tight">{{ $user->name }}</h2>
                        <div class="flex items-center gap-3 mt-2">
                            <span class="px-4 py-1.5 bg-red-50 text-[#EAB308] text-[10px] font-black uppercase tracking-widest rounded-full border border-red-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3 mb-2">
                            <div class="w-8 h-8 bg-[#F1F5F9] rounded-xl flex items-center justify-center border border-[#EFEFEF]">
                                <i data-lucide="info" class="w-4 h-4 text-[#0F172A]"></i>
                            </div>
                            <h4 class="text-sm font-black text-[#0F172A] uppercase tracking-widest">Informasi Dasar</h4>
                        </div>
                        
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-1">Nama Lengkap</label>
                            <input type="text" name="name" value="{{ $user->name }}" required 
                                class="w-full px-6 py-4 rounded-3xl border border-[#EFEFEF] bg-[#F1F5F9] text-sm font-bold text-[#0F172A] focus:ring-4 focus:ring-red-500/5 focus:border-[#EAB308] outline-none transition-all">
                        </div>
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3 mb-2">
                            <div class="w-8 h-8 bg-[#F1F5F9] rounded-xl flex items-center justify-center border border-[#EFEFEF]">
                                <i data-lucide="lock" class="w-4 h-4 text-[#0F172A]"></i>
                            </div>
                            <h4 class="text-sm font-black text-[#0F172A] uppercase tracking-widest">Keamanan Akun</h4>
                        </div>
                ...
                <div class="mt-12 pt-12 border-t border-[#EFEFEF]">
                    <div class="flex items-center gap-3 mb-8">
                        <div class="w-8 h-8 bg-[#F1F5F9] rounded-xl flex items-center justify-center border border-[#EFEFEF]">
                            <i data-lucide="history" class="w-4 h-4 text-[#0F172A]"></i>
                        </div>
                        <h4 class="text-sm font-black text-[#0F172A] uppercase tracking-widest">Aktivitas Saya (10 Terakhir)</h4>
                    </div>

                    <div class="bg-[#F1F5F9] rounded-[32px] border border-[#EFEFEF] overflow-hidden">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-[#EFEFEF]">
                                @forelse($logs as $log)
                                <tr>
                                    <td class="px-8 py-4 text-xs font-bold text-[#0F172A] uppercase tracking-tighter">{{ $log->activity }}</td>
                ...
                <div class="mt-12 pt-8 border-t border-[#EFEFEF] flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <button type="submit" class="w-full md:w-auto bg-[#0F172A] text-white px-12 py-5 rounded-[24px] font-black hover:bg-[#EAB308] transition-all shadow-2xl hover:shadow-red-200 active:scale-[0.98] flex items-center justify-center gap-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div>
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h3 class="text-2xl font-black text-[#0F172A]">Laporkan Masalah Data</h3>
                ...
                <form action="{{ route('profile.report') }}" method="POST" class="space-y-6">
                    @csrf
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-[#0F172A] uppercase tracking-[0.2em] ml-1">Subjek Laporan</label>
                        <input type="text" name="subject" required placeholder="Contoh: Kesalahan Penulisan NIP" class="w-full px-6 py-4 rounded-3xl border border-[#EFEFEF] bg-[#F1F5F9] text-sm font-bold">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-[#0F172A] uppercase tracking-[0.2em] ml-1">Detail Pesan</label>
                        <textarea name="message" rows="4" required placeholder="Jelaskan detail kesalahan data Anda..." class="w-full px-6 py-4 rounded-3xl border border-[#EFEFEF] bg-[#F1F5F9] text-sm font-bold"></textarea>
                    </div>
            ...
            <div>
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h3 class="text-2xl font-black text-[#0F172A]">Riwayat Laporan</h3>
                ...
                <div class="space-y-4 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div class="p-6 bg-[#F1F5F9] rounded-[32px] border border-[#EFEFEF] transition-all hover:shadow-md">
                        <div class="flex justify-between items-start mb-3">
                            <span class="text-xs font-black text-[#0F172A] uppercase tracking-tight">{{ $issue->subject }}</span>
                        ...
                        <div class="mt-4 p-4 bg-white rounded-2xl border border-blue-50">
                            ...
                            <p class="text-[10px] font-bold text-[#0F172A] leading-relaxed">{{ $issue->admin_note }}</p>
                        </div>
                        @endif
